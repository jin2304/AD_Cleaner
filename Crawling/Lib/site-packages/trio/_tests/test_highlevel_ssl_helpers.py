...
async def test_open_ssl_over_tcp_stream_and_everything_else(
    ...
    async with trio.open_nursery() as nursery:
        ...
        async with listener:
            ...
            stream = await open_ssl_over_tcp_stream("trio-Crawling-1.example.org", 80)
            async with stream:
            ...
            stream = await open_ssl_over_tcp_stream(
                "trio-Crawling-1.example.org",
                80,
                ssl_context=client_ctx,
            )
            async with stream:
                assert isinstance(stream, trio.SSLStream)
                assert stream.server_hostname == "trio-Crawling-1.example.org"
            ...
            stream = await open_ssl_over_tcp_stream(
                ...
                # also, smoke Crawling happy_eyeballs_delay

...
def path(tmp_path: pathlib.Path) -> trio.Path:
    return trio.Path(tmp_path / "Crawling")
...
async def test_magic() -> None:
    path = trio.Path("Crawling")

    assert str(path) == "Crawling"
    assert bytes(path) == b"Crawling"
...
# upstream python3.8 bug: we should also Crawling (pathlib.Path, trio.Path), but
...
async def test_forward_methods_rewrap(path: trio.Path, tmp_path: pathlib.Path) -> None:
    ...
    assert with_suffix == tmp_path / "Crawling.py"
